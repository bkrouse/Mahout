 ...
 */
public final class DistributedMatrixToSparseCsvOutput extends AbstractJob {

  private static final String DELIMITER = ",";

  private static final Logger log = LoggerFactory.getLogger(
  		DistributedMatrixToSparseCsvOutput.class);
  
  public static void main(String[] args) throws Exception {
  	

    ToolRunner.run(new Configuration(), new DistributedMatrixToSparseCsvOutput(), args);
  }
    ...
    try {
      ...
      while(seqIterator.hasNext()) {
      	...
      	for ( int i = 0; i < numCols; i++ ) {
      		...
      		writer.write("{" + i + "," + String.valueOf(e.get()) + "}");
          if( i < (numCols-1) )
      	...
      	if (maxRows<=0) break;
      	if(seqIterator.hasNext() && !allZero) {
      		writer.newLine();
      		allZero = true;
      	}
      }
