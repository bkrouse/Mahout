...
public final class NetflixToMatrixConverter {

  private static final Logger log = LoggerFactory.getLogger(NetflixToMatrixConverter.class);
  ...
  private NetflixToMatrixConverter() {
  }

  public static void main(String[] args) throws IOException {

    if (args.length != 4) {
      System.err.println("Usage: NetflixToMatrixConverter /path/to/training_set/ /path/to/qualifying.txt "
    ...
    log.info("Creating map from CustID to RowID at {}/trainingSet/CustIDToRowIDMap.csv", outputPath);
    BufferedWriter writer  = null;
    HashMap<Integer,Integer> custIDMap = new HashMap<Integer,Integer>(500000);
    try {
      FSDataOutputStream outputStream = fs.create(new Path(outputPath, "trainingSet/CustIDToRowIDMap.csv"));
      writer = new BufferedWriter(new OutputStreamWriter(outputStream, Charsets.UTF_8));
      ArrayList<Integer> custIDs = new ArrayList<Integer> (5000000);
      ...
      for (File movieRatings : new File(trainingDataDir).listFiles()) {
        ...
        try  {
          ...
          while (lines.hasNext()) {
            ...
            if(!line.endsWith(MOVIE_DENOTER)) {
              String[] tokens = SEPARATOR.split(line);
              Integer custID = Integer.parseInt(tokens[0]);
              if(!custIDMap.containsKey(custID)) {
                custIDMap.put(custID, 0);
              	custIDs.add(custID);              	
              }
              ratingsProcessed++;
      ...
      log.info("sorting {} CustIDs", custIDs.size());
      Collections.sort(custIDs);
      
      log.info("creating CustIDtoRowID csv");
      
      for(int i=0; i<custIDs.size(); i++)
      {
      	int RowID = i + 1;
      	writer.write(custIDs.get(i) + "," + RowID + NEWLINE);
      	custIDMap.put(custIDs.get(i), RowID);
      }
    ...
    log.info("Creating training set at {}/trainingSet/ratings.csv ...", outputPath);
    Path ratingsPath = new Path(outputPath, "trainingSet/ratings.csv");
    try {
      FSDataOutputStream outputStream = fs.create(ratingsPath);
      writer = new BufferedWriter(new OutputStreamWriter(outputStream, Charsets.UTF_8));
        ...
        try  {
          ...
          while (lines.hasNext()) {
            ...
            if (firstLineRead) {
              ...
              Integer custID = Integer.parseInt(userID);
              Integer rowID = custIDMap.get(custID);
//              writer.write(custID + "," + rowID + ", " + movieID + "," + rating + NEWLINE);
              writer.write(rowID + ", " + movieID + "," + rating + NEWLINE);
              ratingsProcessed++;
              if (ratingsProcessed % 1000000 == 0) {
                log.info("{} ratings processed...", ratingsProcessed);
              }
